...
try:
    ...
    pattern = re.compile(
        r"(?<= |-)i5(?![A-Za-z0-9])"
    )  # i5前面一定是符號-或是空格，後面不是數字和英文字 #re.compile提前編譯，提高效能

    for page in itertools.count(1):  # 語意上比while True更明確
        with req.urlopen(
            f"https://ecshweb.pchome.com.tw/search/v4.3/all/results?cateid=DSAA31&pageCount=100&page={page}"
        ) as resp:
            data = json.load(resp)
        prods = data["Prods"]
        if not prods:
            break  # 不存在頁數則prods為空字串，表示資料讀取完畢，跳出迴圈。
        for prod in prods:
    ...
    if id_and_price_list:
        ...
        sd = (
            sum((obj["Price"] - mean) ** 2 for obj in id_and_price_list)
            / len(id_and_price_list)
        ) ** 0.5
        ...
        with open(
            "standardization.csv", mode="w", newline="", encoding="utf-8"
        ) as csvfile:
