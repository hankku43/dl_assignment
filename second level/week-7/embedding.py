...
def preprocess_raw_data():
    ...
    # 清洗標點符號與空白
    df["ArticleContent"] = df["ArticleContent"].str.replace(
        r"[\s\r\n，。；：？！、]", "", regex=True
    )
    df = df[df["ArticleContent"].str.len() > 0]
    df = df[df["ArticleContent"] != "刪除"]
    
    # 移除不必要的欄位
    df = df.drop(columns=["ArticleType"])
    return df
...
def get_tagged_docs(csv_path):
    ...
    return [
        TaggedDocument(words=row["FilteredContent"].split(), tags=[row["specific_id"]])
...
def train_model(tagged_data):
    """配置參數並訓練模型"""
    print("🚀 開始訓練 Doc2Vec...")
    model = Doc2Vec(vector_size=300, min_count=2, window=5, workers=8, dm=0, dbow_words=1, negative=5, epochs=100)
...
def evaluate_model(model, csv_path, sample_n=100):
    ...
    for row in eval_df.itertuples():
        ...
        sims = model.dv.most_similar([vec], topn=20)

        try:
            rank = [tag for tag, _ in sims].index(str(row.specific_id))
            ranks.append(rank)
        except ValueError:
            ranks.append(999)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PROCESSED_CSV):
        ...
        df_processed = df_processed.drop(columns=["ArticleNo"])
